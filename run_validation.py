...
def classify_topology_nodes(gt_dict, topological_matrix, points, labels, max_allowed_drift_px=None):
    """
    ...
    detected_dict = _make_actual_dict(topological_matrix, points, labels)
...
def compute_topology_statistics(gt_dict, classifications):
    ...
    # Tracks which coordinate tuple keys (r, c) were actually claimed by the tracker
    for key, node_info in classifications.items():
    ...
    # --- ADDED STANDALONE GROUND TRUTH EVALUATION PASS ---
    expected_erasures = 0
    erasure_leaks = 0
    ...
    misclassified = 0
    flipped = 0
    # gt_dict[(r, c)] = (base_bit, pt_x, pt_y, modified_bit)
    for coord_key, gt_info in gt_dict.items():
        base_bit = gt_info[0]
        modified_bit = gt_info[3]

        # 1. Evaluate if a cell is an erasure event (bit flipped or marked as empty)
        is_erased = (modified_bit == -1)

        if is_erased:
            expected_erasures += 1
            # Erasure Leak: A tracking point landed on an explicitly erased grid slot
            if coord_key in classifications:
                erasure_leaks += 1
        else:
            total_visible_targets += 1
            if coord_key in classifications:
                if modified_bit != classifications[coord_key].get("value"):
                    misclassified += 1
                if modified_bit != base_bit:
                    flipped +=1
            else:
                optical_misses += 1
    ...
    stats["misclassified"] = misclassified
    stats["detected"] = len(classifications)

    # matching statistics
    stats["true_positives"] = stats["tp"]
    stats["misalignments"] = stats["ma"]
    stats["ghost_nodes"] = stats["ghost"]
    stats["graph_skips"] = stats["solo"]

    stats["total_visible_targets"] = total_visible_targets
    stats["optical_misses"] = optical_misses
    stats["expected_erasures"] = expected_erasures
    stats["erasure_leaks"] = erasure_leaks

    # recognition indicators
    total_matches = stats["ma"] + stats["tp"] + stats["fp"]
    stats["precision"] = float(stats["tp"]) / total_matches * 100 if total_matches > 0 else 0.0
    stats["recall"] = float(stats["tp"]) / total_visible_targets * 100 if total_visible_targets > 0 else 0.0
    stats["is_passed"] = stats["precision"] > get_precision_threshold()
    return stats
...
def make_blueprint_dict(base_blueprint: np.ndarray, modified_blueprint: np.ndarray,
                        generator,
                        camera,
    ...
    gt_dict = {}
    if modified_blueprint is None:
        modified_blueprint = base_blueprint
    for r in range(H_nodes):
        for c in range(W_nodes):
            ...
            if pt is not None and camera.is_visible(pt):
                ...
                gt_dict[(r, c)] = (base_blueprint[r, c], pt[0], pt[1], modified_blueprint[r,c])
    return gt_dict


def _make_actual_dict(topological_matrix, points, labels):
...
if __name__ == "__main__":
    ...
    with open(os.path.join(DATASET_DIR, "summary.md"), 'w', encoding='ascii') as f:
        ...
        for img_name in case_images:
            ...
            gt_map = {}
            for key, (value, x, y) in parse_ground_truth(gt_path).items():
                gt_map[key] = (value, x, y, value)
            ...
            classifications = classify_topology_nodes(gt_map,
                detected_result.get("topological_matrix", None),
                detected_result["points"], detected_result["labels"])
            ...
            f.write(
                f"| {case_id} | {len(gt_map)} | {len(classifications)} | {misclassified} | {ghosts} | "
